...
include_once(UNIONPAY_PATH. '/lib/utf8/func/log.class.php');
...
function sign(&$params) {
	//global $log;
	Think\Log::write( '=====签名报文开始======' );
	if(isset($params['transTempUrl'])){
		unset($params['transTempUrl']);
	}
	// 转换成key=val&串
	$params_str = coverParamsToString ( $params );
	Think\Log::write( "签名key=val&...串 >" . $params_str );
	
	$params_sha1x16 = sha1 ( $params_str, FALSE );
	Think\Log::write( "摘要sha1x16 >" . $params_sha1x16 );
	// 签名证书路径
	...
	if ($sign_falg) {
		...
		Think\Log::write( "签名串为 >" . $signature_base64 );
		$params ['signature'] = $signature_base64;
	} else {
		Think\Log::write( ">>>>>签名失败<<<<<<<" );
	}
	Think\Log::write( '=====签名报文结束======' );
}
...
//	echo $public_key.'<br/>';
	...
	$params_str = coverParamsToString ( $params );
	Think\Log::write( '报文去[signature] key=val&串>' . $params_str );
	$signature = base64_decode ( $signature_str );
//	echo date('Y-m-d',time());
	$params_sha1x16 = sha1 ( $params_str, FALSE );
	Think\Log::write( '摘要shax16>' . $params_sha1x16 );	
	$isSuccess = openssl_verify ( $params_sha1x16, $signature,$public_key, OPENSSL_ALGO_SHA1 );
	Think\Log::write( $isSuccess ? '验签成功' : '验签失败' );
	return $isSuccess;
}
