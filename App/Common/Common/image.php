...
function markimg($info = array(
	...
	if (!($dst_info[0] >= intval($tj[0]) && $dst_info[1] >= intval($tj[1]))) {
		return false;
	}

	switch ($dst_info[2]) {
	case 1:$dst_im = imagecreatefromgif($dst);
		break;
	case 2:$dst_im = imagecreatefromjpeg($dst);
		break;
	case 3:$dst_im = imagecreatefrompng($dst);
		break;
	case 6:$dst_im = imagecreatefromwbmp($dst);
		break;
	...
	if ($info['str'] != '') {
		...
		switch ($src_info[2]) {
		case 1:$src_im = imagecreatefromgif($src);
			break;
		case 2:$src_im = imagecreatefromjpeg($src);
			break;
		case 3:$src_im = imagecreatefrompng($src);
			break;
		case 6:$src_im = imagecreatefromwbmp($src);
			break;
		default:return ("不支持的文件类型1");
		}
		//支持png本身透明度的方式
	...
	//保存图片
	switch ($dst_info[2]) {
	case 1:imagegif($dst_im, $dst);
		break;
	case 2:imagejpeg($dst_im, $dst);
		break;
	case 3:imagepng($dst_im, $dst);
		break;
	case 6:imagewbmp($dst_im, $dst);
		break;
	default:
		return ("不支持的文件类型2");
	}
