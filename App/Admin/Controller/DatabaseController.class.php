        ...
        if(IS_POST && !empty($tables) && is_array($tables)){ //初始化
            ...
            if(false === $start){ //出错
                ...
                if(isset($tables[++$id])){
                    ...
                    $this->success('<span style="color:#00C700;">备份完成！</span>', '', array('tab' => $tab));
                } else { //备份完成，清空缓存
...
//网站整站打包
	//整站打包
	public function webtozip(){
		$this->meta_title='网站打包';
		if(IS_POST){
			...
			$zip->compress('../'.$dir,array('.svn','DataBak'),$webzipname);
			//发送邮件附件到邮箱
			$result=sendMail(array(
				'to'=>'735579768@qq.com',
				'subject'=>"您的网站{$webzipname}已经打包完成",//主题标题
			...
			$this->success('网站数据备份成功o_0..请单击页面刷新！',U('webtozip'));
		
		}else{
                //列出备份文件列表
                $path = realpath(C('WEBZIPDATA_BACKUP_PATH'));
                $flag = \FilesystemIterator::KEY_AS_FILENAME;
                $glob = new \FilesystemIterator($path,  $flag);

                $list = array();
                foreach ($glob as $name => $file) {
                    if(preg_match('/^\d{8,8}-\d{6,6}-\d+\.web(?:\.gz)?$/', $name)){
                        $name = sscanf($name, '%4s%2s%2s-%2s%2s%2s-%d');
     ...
     * @author 枫叶 <735579768@qq.com>
     */
    public function delwebzip($time = 0){
        if($time){
            $name  = date('Ymd-His', $time) . '-*.web*';
            $path  = realpath(C('WEBZIPDATA_BACKUP_PATH')) . DIRECTORY_SEPARATOR . $name;
            array_map("unlink", glob($path));
            if(count(glob($path))){
                $this->success('备份文件删除失败，请检查权限！');
            } else {
                $this->success('备份文件删除成功！',U('Database/webtozip'));
            }
        } else {
            $this->error('参数错误！');
        }
