  ...
  /**
    ...
    public function uploadfile(){
		...
		if (!empty($_FILES)) {
			...
			$foldertype='';
			switch($extend[$va]){
				case 'jpg': $targetFolder.='/file/image'; break;
				case 'png': $targetFolder.='/file/image'; break;
				case 'gif': $targetFolder.='/file/image'; break;
				case 'jpeg': $targetFolder.='/file/image';break;
				case 'bmp': $targetFolder.='/file/image';break;
    ...
    public function uploadpic(){
		...
		if (!empty($_FILES)) {
			...
			$foldertype='';
			switch($extend[$va]){
				case 'jpg': $targetFolder.='/image';$foldertype='/thumb'; break;
				case 'png': $targetFolder.='/image';$foldertype='/thumb'; break;
				case 'gif': $targetFolder.='/image';$foldertype='/thumb'; break;
				case 'jpeg': $targetFolder.='/image';$foldertype='/thumb'; break;
				case 'bmp': $targetFolder.='/image';$foldertype='/thumb'; break;
				case 'mp4': $targetFolder.='/file/video'; break;
				default: $targetFolder.='/file/other'; break;
				}
	...
	function createthumb(){
		if(IS_POST){
			...
			if(empty($thumbpath)){
				...
				$temarr=array();
				$map['pic']=array('neq',0);
				if($arccatid!=0)$map['category_id']=$arccatid;
				$list=M('Article')->where($map)->field('pic')->select();
				
				foreach($list as $val)$temarr[]=$val['pic'];
				
				if($catid!=0)$map['category_id']=$catid;
				$list=M('goods')->where($map)->field('pic,xc')->select();
				foreach($list as $val){
					$temarr[]=$val['pic'];
					//拆分相册
					$xcarr=explode('|',$val['xc']);
					foreach($xcarr as $val)$temarr[]=$val;
				}
				//把查到的数量保存到缓存中
				F('thumbpath',$temarr);
				$thumbpath=$temarr;
					...
					if(count($thumbpath)>0){
						...
						$dpath=str_replace('image/','image/thumb/',$spath);
							if(file_exists($spath)){
								//源文件存在
									if(file_exists($dpath))unlink($dpath);
