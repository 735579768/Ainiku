...
class CutpictureController extends AdminController {
	public function init() {
		...
		$ShearPhoto["config"] = array(
			...
			"temp"         => DATA_DIR_PATH . "cutpicturefile/temp", //等待截图的大图文件。就是上传图片的临时目录，截图后，图片会被删除
			"tempSaveTime" => 600, //临时图片（也就是temp内的图片）保存时间，需要永久保存请设为0。单位秒
			"saveURL"      => __ROOT__ . "/Uploads/image/cutfile/" . date('Ymd'), //后面不要加斜杠，系统会自动给补上！不要使用中文
			"filename"     => uniqid("cutpicture_") . "_" . mt_rand(100, 999) . "_", //文件名字定义！要生成多个文件时 系统会自动在后面补0 1 2  3.....;
		);
		return $ShearPhoto["config"];
	}
	public function cutp() {
		$ShearPhoto["config"] = $this->init();
		...
		} else //切图成功时
		{
			...
			foreach ($result as $key => $val) {
				$result[$key]['status'] = 1;
				$imgurl                 = str_replace(array('./', '\\', __ROOT__), array('/', '/', ''), $val['ImgUrl']);
				$data['path']           = $imgurl;
				$data['sha1']           = sha1_file();
	...
	function uploadf() {
		...
		$ini_set              = array(
			...
			'out_time'  => 20, //上传超时设置
			'list'      => $ShearPhoto["config"]["temp"] . '/', //上传路径
			'whitelist' => array(
				".jpeg",
				".gif",
				".png",
		...
		*/
		echo ('{"success":"' . str_replace(array("\\\\", "\/", ShearURL, "\\", './'), array("\\", "/", "", "/", '/'), __ROOT__ . $UpFile['file_url']) . '"}');
		die();
	}
	function errobug() {
		$e = error_get_last();
...
class ShearPhoto {
	...
	public function run($JSconfig, $PHPconfig) {
		$tempurl = $PHPconfig["temp"] . '/' . "shearphoto.lock";
	...
	protected function CreateArray($PHPconfig, $JSconfig, $strtype) {
		...
		foreach ($PHPconfig["width"] as $k => $v) {
			...
			$file_url                        = $PHPconfig["saveURL"] . '/' . $PHPconfig["filename"] . $k . $strtype;
