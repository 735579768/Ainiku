...
//		$member_group_id=$uinfo['member_group_id'];
		$map.=' and uid='.UID;
		if(!empty($order))$map.=' and ordernum_id=\''.$order.'\'';
		if(!empty($username))$map.=' and a.username=\''.$username.'\'';
	...
	public function tongji(){
		...
		if($status!=-1)$map.=' and __DB_PREFIX__order.status='.$status;
		if($starttime!=$endtime){
		if(!empty($starttime) && !empty($endtime)){
		...
		if(!empty($title))$map.=' and  b.title like \'%'.$title.'%\'';
		if(!empty($order))$map.=' and ordernum_id=\''.$order.'\'';
		if(!empty($username))$map.=' and a.username=\''.$username.'\'';
...
//    }
    ...
    function delsingle($order_id=null){
		...
		if(empty($order_id))$this->error('请先进行选择');
...
function dopay($order_pay_id=null){
	if(empty($order_pay_id))$this->error('非法访问');
	$info=M('OrderPay')->find($order_pay_id);
	if(empty($info))$this->error('非法访问');
	$total='yes';
	$obi='yes';
	//如果有金额的话去网银支付
	...
	if($info['status']!='0')$this->error('已经支付,请不要重复操作!');
	if(IS_POST){
		if($total=='no' || $obi=='no')$this->error('账户余额不足!');
		//符合条件进行支付
