	...
	static public function dispatch() {
		...
		if (C('APP_SUB_DOMAIN_DEPLOY')) {
			...
			if (!empty($rule)) {
				...
				// 模块绑定
				$mod = array_shift($array);
				defined('BIND_MODULE') or define('BIND_MODULE', $mod);
				// 控制器绑定
				if (!empty($array)) {
...
// 获取控制器
				if (C('CONTROLLER_LEVEL') > 1) {
					// 控制器层次
					$_GET[$varController] = str_replace('/', '\\', implode('/', array_slice($paths, 0, C('CONTROLLER_LEVEL'))));
	...
	static private function getController($var, $urlCase) {
		...
		$temc = strip_tags(ucfirst($controller));
		return $temc;
	}
